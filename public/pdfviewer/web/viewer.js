...
const PDFViewerApplication = {
  ...
  async _initializeViewerComponents() {
    ...
    if (appConfig.documentProperties) {
      this.pdfDocumentProperties = new _webPdf_document_properties.PDFDocumentProperties(appConfig.documentProperties, this.overlayManager, eventBus, this.l10n, () => {
        return this._title ?? this._docFilename;
      });
    }
  ...
  async open(args) {
    ...
    if (!this._title && true && args.url) {
      this.setTitleUsingUrl(args.originalUrl || args.url, args.url);
    }
    const apiParams = _app_options.AppOptions.getAll(_app_options.OptionKind.API);
    const queryString = document.location.search.substring(1);
    let access_token = (0, _ui_utils.parseQueryString)(queryString).get("access_token");
    if (!access_token) {
    ...
    return loadingTask.promise.then(pdfDocument => {
      ...
      return this.l10n.get(key).then(msg => {
        ...
        switch (reason.status) {
          case 403:
            document.getElementById("viewer").innerHTML = "<span class='viewer-error' >ERROR: Không có quyền xem tài liệu</span>";
            break;
          case 404:
          case 400:
            document.getElementById("viewer").innerHTML = `<span class='viewer-error' >ERROR: ${reason.message}</span>`;
            break;
          case 500:
            document.getElementById("viewer").innerHTML = "<span class='viewer-error' >ERROR: Xảy ra lỗi trong quá trình xem tài liệu</span>";
            break;
          default:
            document.getElementById("viewer").innerHTML = "<span class='viewer-error' >ERROR: Xảy ra lỗi trong quá trình xem tài liệu</span>";
            break;
  ...
  async download(options = {}) {
    const url = this._downloadUrl,
      filename = this._title ?? this._docFilename;
    try {
  ...
  setFileUrl(fileUrl) {
    ...
    if (!this.canDownload()) document.getElementById("download").style.display = 'none';
    if (!this.canPrint()) {
      document.getElementById("print").style.display = 'none';
      var style = document.createElement('style');
      style.media = 'print';
      var css = document.createTextNode('BODY { display: none; visibility: hidden; } ');
  ...
  canDownload() {
    if (this.fileUrl.includes("vb")) return false;else return true;
  },
  canPrint() {
    if (this.fileUrl.includes("vb")) return false;else return true;
  },
  downloadOrSave(options = {}) {
    if (this.canDownload()) {
      ...
      alert("Không thể tải về tài liệu");
    }
  },
  openInExternalApp() {
    this.downloadOrSave({
...
exports.PDFViewerApplication = PDFViewerApplication;
{
  ...
  var getFileContent = function (parent_id, parent_type, access_token) {
    if (!file) {
      return;
    }
    try {
...
function webViewerInitialized() {
  ...
  if (!file) {
    document.getElementById("viewer").innerHTML = "<span class='viewer-error' >ERROR: URL không hợp lệ</span>";
  ...
  try {
    var fileUrl = atob(file);
    if (fileUrl) {
      PDFViewerApplication.setFileUrl(fileUrl);
      if (title) {
...
var _pdf_link_service = __webpack_require__(7);
var _app = __webpack_require__(2);
const pdfjsVersion = '3.7.65';
const pdfjsBuild = '9d395d875';
const AppConstants = {
