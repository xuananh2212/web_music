...
import AddEditAlbum from "../components/add-edit-user-history/AddEditUserHistory";
...
const useUserHistoryQuery: BaseQueryTable = () => {
  ...
  const { isPending: isPendingDelete, mutateAsync: mutateDeleteAsync } = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({
        queryKey: [MUSIC_QUERY_KEY_ENUM.USER_HISTORY],
      });
    },
  });
  ...
  const { isPending, refetch, data } = useQuery({
    queryKey: [MUSIC_QUERY_KEY_ENUM.USER_HISTORY, queryParams],
    queryFn: async () => {
      const response = await musicService.getUserHistories(queryParams);
